...
void *IreadStrided ( void *thread_arg )
{
	int error_code;
	int err;
	...
	err = UPC_ADIO_ReadStrided( ptr->fd,
	                            ptr->mem_list_count,
	                            ptr->mem_ptrs,
	                            ptr->mem_lengths,
	                            ptr->file_list_count,
	...
	//Set the ret pointer to contain -1 on error, or the size read on success
	*ptr->ret = err;

	return (void *) ptr;
}

void UPC_ADIO_IreadStrided ( Plfs_fd *fd,
                             uint32_t mem_list_count,
                             void* mem_ptrs,
                             uint64_t* mem_lengths,
                             uint32_t file_list_count,
...

{
	...
	iret = pthread_create( &io_thread, NULL, IreadStrided, (void *)&arg );
