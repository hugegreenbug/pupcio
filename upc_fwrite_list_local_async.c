...
/*                       E-mail: yyy@gwu.edu                            */
/************************************************************************/
// upc_fwrite_list_local_async.c
#include <upc.h>
#include <stdlib.h>
#include "upc_io.h"

void upc_all_fwrite_list_local_async( upcio_file_t *fh_shared,
                                      ...
                                      upc_flag_t sync_mode )
{
	...
	UPC_ADIO_IwriteStrided( fd, memvec_entries, mem_ptrs, mem_lengths, filevec_entries, 
				file_offsets, file_lengths, &request, ret, &error_code );

	/*------------------------------------------------------------------*/
	/* Setup metadata                                                   */
	/*------------------------------------------------------------------*/
	fh->request = request;
	fh->async_op = __REF_UPC_WRITE_LIST_LOCAL_ASYNC;
	fh->size = size;
