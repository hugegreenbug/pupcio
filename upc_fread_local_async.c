...
/*                       E-mail: yyy@gwu.edu                            */
/************************************************************************/
// upc_fread_local_async.c
#include <upc.h>
#include <stdio.h>
#include "upc_io.h"

void upc_all_fread_local_async( upcio_file_t *fh_shared,
                                void *buffer,
                                upc_off_t size,
                                uint32_t nmemb,
				int64_t *ret,
                                upc_flag_t sync_mode )
{
	...
	count = size*nmemb;
	UPC_ADIO_IreadContig( fd, buffer, count, fh->private_pointer, 
	...
	/*------------------------------------------------------------------*/
	fh->request = request;
	fh->async_op = __REF_UPC_READ_LOCAL_ASYNC;
	fh->size = count;
