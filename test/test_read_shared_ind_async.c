...
int main()
{
	...
	if(!MYTHREAD)
		printf("upcio test: test fread_shared_ind_async with %d Threads\n", THREADS);
	...
	upc_barrier;
	upc_all_fread_shared_async(fd, (shared void *)buffer, blocksize, size, nmemb, 
				   &ret_size, sync_mode);
	dummy = NULL;
	...
	if(!MYTHREAD) {
		printf("upcio test: Done with fread_shared_ind_async testing\n");
		upc_free(buffer);
	}
