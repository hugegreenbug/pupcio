...
int main()
{
	...
	if(!MYTHREAD)
		printf("upcio test: test fread_shared_com_async with %d Threads\n", THREADS);
	...
	upc_barrier;
	upc_all_fread_shared_async(fd, (shared void *)buffer, blocksize, size, 
	...
	if(!MYTHREAD)
	{
		printf("upcio test: Done with fread_shared_com_async testing\n");
		upc_free(buffer);
	}
