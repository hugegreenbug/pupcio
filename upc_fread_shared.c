...
int64_t upc_all_fread_shared( upcio_file_t *fh_shared,
			      shared void *buffer,
			      uint32_t blocksize,
			      upc_off_t size,
			      uint32_t nmemb,
			      upc_flag_t sync_mode )
{
	...
	upc_off_t count, nblocks, blocksize_byte, roundsize;
	int round;
	int start_th, my_th;
	int i, error_code;
	...
	if( fh->flags & UPC_INDIVIDUAL_FP )
	{
		if( blocksize )
		{
			...
			UPC_ADIO_ReadContig( fd, local_buf, count, fh->private_pointer, &error_code );
			...
			{
				upc_memput(buffer_char+round*roundsize+my_th-start_th, &local_buf[i], blocksize_byte);
				my_th++;
				if(my_th == THREADS)
				{
					my_th = 0;
			...
			upc_memput(buffer_char+round*roundsize+my_th-start_th, &local_buf[i], count-i);
		...
		else
		{
			...
			upc_memput(buffer, local_buf, count);
			free(local_buf);
		}
		/*------------------------------------------------------------------*/
	...
	else
	{
		if( blocksize )
		{
			...
			UPC_ADIO_ReadStrided( fd, 1, local_buf, &mpi_size, round, disparray, dispsize, &error_code );
			free(disparray);
			free(dispsize);
			if(error_code != UPC_ADIO_SUCCESS)
			{
		...
		else
		{
			...
			local_buf = (unsigned char *)malloc(sizeof(unsigned char)*count);
			if( MYTHREAD == 0 )
				UPC_ADIO_ReadContig( fd, local_buf, count, fh->shared_pointer, &error_code );
			else
				UPC_ADIO_ReadContig( fd, local_buf, 0, fh->shared_pointer, &error_code );
			if(error_code != UPC_ADIO_SUCCESS)
			{
				/* Error handling here */
				return -1;
